"""
Validation script for CTD-YOLO
"""
import argparse
import json
import os
...
from utils import (check_img_size, non_max_suppression, scale_coords, 
                   xyxy2xywh, xywh2xyxy, clip_coords, colorstr)
...
def run(data,
    ...
    """
    Run validation on CTD-YOLO model
    """
    ...
    if pt or jit:
        ...
        print(f'{colorstr("WARNING:")} --half not compatible with --dynamic, --optimize or TensorRT exports.\n'
              f'Running inference at --half=False.')
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], imgsz, batch_size, gs, single_cls, pad=pad, rect=rect,
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        if opt.conf_thres > 0.001:  # https://github.com/ultralytics/yolov5/issues/1466
            print(f"WARNING: confidence threshold {opt.conf_thres} > 0.001 produces invalid results ⚠️")
        run(**vars(opt))
...
def attempt_load(weights, device=None, inplace=True, fuse=True):
    # Load model
    model = CTD_YOLO(weights, device=device)
...
def process_batch(detections, labels, iouv):
    return torch.zeros(detections.shape[0], iouv.shape[0], dtype=torch.bool)
